...
# certificate requests from 8 (4 clusters × 2 certs) to just 2 (wildcard-net, wildcard-ai).
#
# Secrets to copy:
...
copy_secret() {
    ...
    if ! kubectl --context="${source_ctx}" get secret -n "${namespace}" "${secret_name}" >/dev/null 2>&1; then
        echo "    ⚠️  Secret ${namespace}/${secret_name} not found on ${source_ctx}, skipping..."
        return 1
    fi
...
if [[ -n "${TARGET_CLUSTER}" ]]; then
    ...
    if [[ ! " ${DOWNSTREAM_CLUSTERS[*]} " =~ " ${TARGET_CLUSTER} " ]]; then
        echo "❌ Error: ${TARGET_CLUSTER} is not a downstream cluster."
    ...
    echo "📋 Syncing certificates from ${SOURCE_CLUSTER} to all downstream clusters..."
